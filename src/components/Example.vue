      ...
      <div class="col m12 card-panel">
        <form @submit.prevent="agregarUsuario">
          ...
          <div class="row">
            ...
            <div class="col m4">
              ...
              <select v-model="estado_civil">
                ...
                <option v-for="estado in estados_civiles" v-bind:key="estado" v-bind:value="estado.id">{{
          ...
          <div class="row">
            <form @submit.prevent="agregarPasatiempo">
              <div class="col m4 card-panel">
                ...
                <button type="submit" class="btn indogp darken-3"> AGREGAR PASATIEMPO <i class="material-icons">send</i>
                </button>
                <br>
                <ul>
                  <li v-for="(pasatiempo, index) in pasatiempos" v-bind:key="pasatiempo"> {{ pasatiempo.id }} - {{
                    pasatiempo.description }} <a href="#!" @click="pasatiempos.splice(index, 1)"><i
          ...
          <div class="row">
            <div class="col m4">
              <label>
                <input type="checkbox" v-model="suscrito">
                <span>suscribirse al boletin de noticias</span>
              </label>
            </div>
          </div>
          <div class="row">
            <button type="submit" class="btn indigo darken-4">{{(indice == -1 ? 'AGREGAR' : 'ACTUALIZAR')}} USUARIO <i
        ...
        <table class="table bordered striped">
          ...
          <tbody>
            <tr v-for="(usuario, index) in usuarios" v-bind:key="usuario"  v-bind:class="{'indigo darken-4 white-text': index == indice}">
              ...
              <td>{{ usuario.edad }}</td>
              <td>{{ usuario.estado_civil_description }}</td>
  ...
  methods: {
    agregarUsuario() {
      ...
      var data = {
        ...
        correo: this.correo,
        estado_civil_description: this.estados_civiles[index_estado_civil].description,
    ...
    eliminar(index) {
      if (!confirm('Desea elimina este registro')) return;
    ...
    editar(index) {
      ...
      this.psatiempos = usuario.pasatiempos;
    }
  }
}
</script>
